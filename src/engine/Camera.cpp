...
void Camera::TreatMouseRotation(GLFWwindow* window, double deltaTime)
{
	...
	if (abs(glm::angle(newForward, vecUp) - glm::radians(90.0f)) <= glm::radians(85.0f))
	{
		forward = newForward; // Use the new rotation for the pitch
	}

	forward = glm::rotate(forward, glm::radians(-rotY), vecUp); // Use the new rotation for the yaw
